...
namespace ComPort
{
    ...
    // Detect available MOUSE and RECEIVER COM ports
bool detectDevices(std::wstring &mouseComPort, std::wstring &receiverComPort)
{
    ...
    for (DWORD i = 0; SetupDiEnumDeviceInfo(deviceInfoSet, i, &deviceInfoData); i++)
    {
        ...
        if (SetupDiGetDeviceRegistryProperty(deviceInfoSet, &deviceInfoData, SPDRP_HARDWAREID, NULL, (BYTE *)hardwareId, sizeof(hardwareId), NULL))
        {
            ...
            std::wcout << L"Detected Hardware ID: " << hwId << std::endl;

            WCHAR busDesc[256] = {0};
            DEVPROPTYPE propType;
            if (SetupDiGetDeviceProperty(deviceInfoSet, &deviceInfoData, &DEVPKEY_Device_BusReportedDeviceDesc, &propType, (BYTE *)busDesc, sizeof(busDesc), NULL, 0))
            {
                std::wstring busReportedDesc(busDesc);
                std::wcout << L"Bus Reported Device Description: " << busReportedDesc << std::endl;

                if (hwId.find(vidPid) != std::wstring::npos && busReportedDesc == mouseDescription)
                {
                    ...
                    if (hKey != INVALID_HANDLE_VALUE)
                    {
                        ...
                        {
                            mouseComPort = portName;
                        }
                        RegCloseKey(hKey);
                    }
                }
                else if (hwId.find(vidPid) != std::wstring::npos && busReportedDesc == receiverDescription)
                {
                    WCHAR portName[256] = {0};
                    DWORD size = sizeof(portName);
                    HKEY hKey = SetupDiOpenDevRegKey(deviceInfoSet, &deviceInfoData, DICS_FLAG_GLOBAL, 0, DIREG_DEV, KEY_READ);
    ...
    bool connect(Subject targetSubject, std::wstring targetComPort)
    {
        ...
        {
            std::cerr << "No COM port found for " << (targetSubject == Subject::MOUSE ? "MOUSE" : "RECEIVER") << std::endl;
            return false;
        }
    ...
    void disconnect()
    {
        ...
        connectedTo = ComPort::Subject::UNKNOWN;
        read_data_X = nullptr;
    }
    ...
    bool set_device(Subject &subject)
    {
        ...
        if (std::regex_search(line, match, pattern))
        {
            ...
            if (line.find("MOUSE") != std::string::npos)
            {
                ...
                read_data_X = read_data_mouse;
                std::cout << "[app_scan_and_detect] Handler set to read_data_mouse.\n";
            }
            else if (line.find("RECEIVER") != std::string::npos)
            {
                subject = Subject::RECEIVER;
                read_data_X = read_data_receiver;
        ...
        {
            std::cerr << "[app_scan_and_detect] Pattern not found in response.\n";
            subject = Subject::UNKNOWN;
            read_data_X = nullptr;
            return false;
        }
